import { createState, createFuseVector, vectorFuse } from '../src'
...
describe(`vectorFuse`, () => {
  ...
  test(`can reorder vector sources`, () => {
    const [stateSource1, setState1] = createState(createFuseVector([1, 2]))
    const [stateSource2] = createState(3)
    const source = vectorFuse((use) => {
      const item = use(stateSource1)
  ...
  test(`can combine vector sources`, () => {
    ...
    // TODO: fix this order
    expect(output).toEqual([createFuseVector([3, 6, 4, 8])])
  })

  test(`can reorder combined vector sources`, () => {
    const [stateSource1, setState1] = createState(createFuseVector([1, 2]))
    const [stateSource2, setState2] = createState(createFuseVector([3, 4]))
    const source = vectorFuse((use) => {
      const item = use(stateSource1)
    ...
    setState1(createFuseVector([2, 1]))
    expect(output.reverse()).toEqual([
      createFuseVector([3, 6, 4, 8]),
      createFuseVector([6, 3, 8, 4]),
    ])
  })

  test.skip(`only re-runs the fusor as required`, () => {})
})
