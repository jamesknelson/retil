...
> extends IssueMessageDefinitions<I> {
  ...
  validateOnChange?: PropNamesFor<I>
  validateOnInit?: PropNamesFor<I>
}
...
> {
   ...
   * all props will be validated. Returns `true` if the specified props pass
   * validation.
   *
   * Call this function from your blur or submit handlers to perform validation
   * after the user has completed input on a field, or call it from your
   * operation to do a final check before submitting a form.
   *
   ...
   * been unmounted, then a warning will be logged.
   */
  validate(propNames?: PropNamesFor<I> | PropNamesFor<I>[]): boolean
}
